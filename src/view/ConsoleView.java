...
public class ConsoleView implements View {
    ...
    public MainMenuCommand showMainMenuWithResult() {
        String menu = String.format(
                ...
                MainMenuCommand.LIST_ANIMALS.ordinal(),
                MainMenuCommand.LIST_ANIMALS.getTag(),
                MainMenuCommand.EXIT.ordinal(),
                MainMenuCommand.EXIT.getTag());
        printLineWithSymbol("=", SIZE_LINE);
        printColorLine("Меню питомника:", GREEN_COLOR);
        ...
        while (true) {
            try {
                ...
                printColorLine("Неправильный пункт меню!", RED_COLOR);
            }
        }
    }
    ...
    public boolean showAddAnimalDialog() {
        String infoMessage = "Введите параметры животного в виде строки: \"Имя День рождения Вид животного\"\n" +
                "День рождения в формате: dd-mm-yyyy (12-03-2022): \n" +
                "Вид животного может принимать значения: " + Arrays.asList(AnimalGenius.values());
    ...
    /**
     * Входные данные при добавлении животного
     ...
     * @return возвращает record AnimalData, содержащую правильные параметры животного
     */
    ...
    private void printAnimalInfo(AbstractAnimal animal) {
        ...
        if (animal.getAnimalSkills().isEmpty()){
            System.out.println("Умений пока нет");
        } else {
            System.out.printf("Умения: %s\n", animal.getAnimalSkills().toString().replaceAll("^\\[|\\]$", ""));
        }
//        printLineWithSymbol("~", SIZE_LINE);
    }
}